...
for experiment in experiments:
    ...
    if experiment['balancing'] == 'Loss':
        ...
        history = model.fit(trainingData, trainingLabels, epochs=150, batch_size=64, validation_data=(validationData, validationLabels), class_weight=dataset.classWeights, callbacks=checkPoint)
    else:
        checkPointFilePath += 'Augmentation'
        checkPoint = callbacks.ModelCheckpoint(checkPointFilePath, save_best_only=True)
        history = model.fit(balancedTrainingData, balancedTrainingLabels, epochs=150, batch_size=64, validation_data=(validationData, validationLabels), callbacks=checkPoint)
